...
class FullScreenApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # GAIN input
        self.gain_label = QLabel("RX gain [dB] (0 to 70 dB)")
        self.gain_label.setAlignment(Qt.AlignLeft)
        self.left_layout.addSpacing(5)
        #self.gain_label.setContentsMargins(50, 0, 0, 0)
        self.gain_label.setFont(font)
        self.left_layout.addWidget(self.gain_label)

        self.gain_input = QLineEdit()
        self.gain_input.setAlignment(Qt.AlignLeft)
        #self.gain_input.setContentsMargins(50, 0, 0, 15)
        self.gain_input.setFont(input_font)
        self.gain_input.setText("10")  # default value
        self.gain_input.setFixedWidth(250)
        self.gain_input.setAlignment(Qt.AlignLeft)
        self.left_layout.addWidget(self.gain_input)
        ...
        # Disable inputs if Pluto not connected
            ...
            self.gain_input.setDisabled(True)
        ...
        self.left_layout.addSpacing(10)
        self.close_button.setFixedSize(250, 30)
    ...
    def get_wifi_ssid_signal(self):
        try:
            self.output = subprocess.check_output(['iwconfig', 'wlo1']).decode('utf-8')
            self.signal_level = re.search(r"Signal level=(-\d+)", self.output)
            if self.signal_level:
                return int(self.signal_level.group(1))
            else:
                return 0
    ...
    def start_measurement(self):
        if  self.not_connected:
            print("Starting Wi-Fi measurement...")
            ...
            try:
                ...
                return
            print("Pluto SDR measurement started.")
        self.stop_requested = False 
        print("Not connected: ", self.not_connected)    
        #-----ROS initialize-----
        self.x = np.array([0.0])
        self.y = np.array([0.0])
        self.amps = np.array([0.0])
        rclpy.init()
        ...
        self.ros_timer.start(100)  # start live updates
        print("ROS node initialized and subscription created.")
    ...
    def update_live_plot(self):
        ...
        #print("Updating live plot...")
        #print("Not connected: ", self.not_connected)

        if self.not_connected: 
            # --- Get Wi-Fi signal strength ---
            #print("Wi-Fi meritev start")
            self.amps = np.append(self.amps, self.get_wifi_ssid_signal())
            #print("Wi-Fi Signal Strength (dBm): ", self.amps)
        else: 
            #--- Receive samples ---
            samples = self.sdr.rx()
            ...
            f = np.linspace(float(self.samp_input.text())/-2, float(self.samp_input.text())/2, len(psd))
            # Plot freq domain
            plt.figure(1)
            ...
            self.amps = np.append(self.amps, P_dB)
            #print("Power dB: ", P_dB)
        ...
        self.ax.set_ylim(-10, 10)
        self.ax.set_aspect('equal')
        scatter = self.ax.scatter(self.x, self.y, c=self.amps, cmap='plasma', vmin=-100, vmax=10, marker='o')
    ...
    def pose_callback(self, msg):
        ...
        self.x = np.append(self.x, msg.transforms[0].transform.translation.x)
        self.y = np.append(self.y, msg.transforms[0].transform.translation.y)

        #print("Updating plot from pose callback...")
